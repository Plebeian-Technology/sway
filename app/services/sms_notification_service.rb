...
class SmsNotificationService
  def self.send_bill_of_the_week_notification(bill)
    ...
    message = "New Bill of the Week in #{bill.sway_locale.city.titleize}: #{bill.title}. Check it out on Sway!"
  ...
  def self.send_voting_reminder(user, bill)
    message = "Reminder: You haven't voted on the Bill of the Week in #{bill.sway_locale.city.titleize}: #{bill.title}. Vote now on Sway!"
    SmsDeliveryJob.perform_later(to: user.phone, body: message)
  end
end
