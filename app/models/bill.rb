...
class Bill < ApplicationRecord
    ...
    def self.of_the_week(sway_locale:)
        b =
            where(sway_locale:, scheduled_release_date_utc: Time.zone.today - 1.month..Time.zone.today)
    ...
    sig do
        params(current_user: T.nilable(User), current_sway_locale: T.nilable(SwayLocale)).returns(
    ...
    def render(current_user, current_sway_locale)
        {
            ...
            user_vote: UserVote.find_by(user: current_user, bill_id: id)&.attributes,
            bill_score: bill_score&.to_sway_json,
            districts:
                current_user&.districts(current_sway_locale)&.map(&:to_sway_json) ||
                    [current_sway_locale&.at_large_district&.to_sway_json].compact,
        }
    end
    ...
    def downcase_status
        ...
        if STATUSES.include?(s.downcase)
            ...
            Rails.logger.warn(
                "Bill.downcase_status - received status of #{s} is NOT valid. Should be one of #{STATUSES.join(", ")}",
            )
            self.status = nil
        end
    end
    ...
    def updated_scheduled_release_date_utc?
        scheduled_release_date_utc == Time.zone.today &&
            attribute_before_last_save("scheduled_release_date_utc") != Time.zone.today
    end
