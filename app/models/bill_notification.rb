...
class BillNotification < ApplicationRecord
  ...
  sig { void }
  def notify
    Rails.logger.info("BillNotification.notify - sending notifications for bill - #{bill.id} / #{bill.sway_locale.name}")
    SwayPushNotificationService.new(
      title: "New Bill of the Week",
      body: "#{bill.sway_locale.city_name}: #{bill.title}"
    ).send_push_notification
  rescue Exception => e # rubocop:disable Lint/RescueException
    Rails.logger.warn("BillNotification.notify - ERROR sending notifications for bill - #{bill.id} / #{bill.sway_locale.name}")
