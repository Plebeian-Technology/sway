class UserOrganizationMembershipsController < ApplicationController
    ...
    inertia_share do
...

        {
            ...
            user: {
                **u,
                memberships: current_user.user_organization_memberships&.map(&:to_sway_json),
            },
        }
    end
    ...
    def show
        ...
        if @current_user_membership.admin?
            props[:members] = members.map do |m|
                { id: m.id, user_id: m.user.id, full_name: m.user.full_name, email: m.user.email, role: m.role }
            end
            props[:pending_changes] = pending_changes.map(&:to_sway_json)
        else
            props[:pending_changes] = pending_changes.where(updated_by: current_user).map(&:to_sway_json)
        end
    ...
    def update
        ...
        unless @current_user_membership&.admin?
            ...
            redirect_to user_organization_membership_path(@current_user_membership) and return
        end

        if working_membership.update(role: UserOrganizationMembership.roles.fetch(params[:role]))
    ...
    def pending_changes
        OrganizationBillPositionChange.includes(organization_bill_position: :organization).where(
    ...
    def working_membership
        @working_membership ||= UserOrganizationMembership.find(params[:working_membership_id])
    end

    def set_current_user_membership
        @current_user_membership = UserOrganizationMembership.includes(:organization).find(params[:id])
    end
end
