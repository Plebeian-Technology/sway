...
class ApplicationController < ActionController::Base
    ...
    inertia_share do
        {
            user: current_user&.to_sway_json&.merge({ address: current_user&.address&.attributes }),
    ...
    def is_api_request_and_is_route_api_accessible?
        if request.path.starts_with?("/api/admin/")
            unless authenticate_with_api_key! && current_user&.is_admin?
                render json: { message: "Missing API Key. Include it an Authorization header." }, status: :accepted
            end
