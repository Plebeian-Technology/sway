...
class UserVotesController < ApplicationController
    ...
    def create
        uv = UserVote.find_or_initialize_by(user: current_user, bill_id: user_vote_params[:bill_id])
        uv.support = user_vote_params[:support]
        uv.save

        redirect_to(bill_path(user_vote_params[:bill_id]), inertia: { errors: uv.errors })
    end
