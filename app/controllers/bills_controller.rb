...
class BillsController < ApplicationController
    ...
    def edit
        ...
        render_component(
            ...
            {
                bills: Bill.current_session(current_sway_locale)&.map(&:to_sway_json),
                bill: @bill.to_sway_json.tap { |b| b[:organizations] = @bill.organizations.map(&:to_sway_json) },
                legislators:
                    current_sway_locale
                        &.legislators
                        &.filter do |l|
    ...
    def create
        ...
        if b.save
            ...
            redirect_to new_bill_path({ event_key: "bill" }), inertia: { errors: b.errors }
        end
    ...
    def update
        ...
        if @bill.update(bill_params.except(*vote_params))
            ...
            redirect_to edit_bill_path(@bill.id, { event_key: "bill" }), inertia: { errors: @bill.errors }
        end
    end
...
:sway_locale).find(params[:id]),
    ...
    def remove_audio(audio_path)
        ...
        delete_file(bucket_name: SwayGoogleCloudStorage::BUCKETS[:ASSETS], file_name: audio_path)
    end
    ...
    def legislator_vote_params
        params
            ...
            .map { |p| p.transform_keys(&:underscore).permit(:legislator_id, :bill_id, :support) }
    end
