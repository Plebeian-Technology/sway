...
class OrganizationBillPositionsController < ApplicationController
  ...
  def index
    if params[:bill_id]
      render json: OrganizationBillPosition.where(bill_id: params[:bill_id]),
             status: :ok
    elsif params[:organization_id]
      render json:
               OrganizationBillPosition.where(
                 organization:
  ...
  def show
    render json:
             OrganizationBillPosition.find_by(
  ...
  def create
    if organization_bill_positions_params[:positions].present?
      OrganizationBillPosition.where(
        bill_id: organization_bill_positions_params[:positions].first[:bill_id],
      ).destroy_all
  ...
  def organization_bill_positions_params
    params.require(:organization_bill_position).permit(
      positions: %i[bill_id organization_id support summary],
    )
  end
