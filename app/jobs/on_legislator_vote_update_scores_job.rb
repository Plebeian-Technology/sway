...
class OnLegislatorVoteUpdateScoresJob < ApplicationJob
  ...
  def perform(newly_saved_legislator_vote, previous_support)
    if newly_saved_legislator_vote.support == previous_support
      Rails.logger.warn("OnLegislatorVoteUpdateScoresJob.update_user_legislator_scores - SHOULD NOT BE TRIGGERED. newly_saved_legislator_vote.suport should NEVER == previous_support. LegislatorVote: #{newly_saved_legislator_vote.id}, previous_support: #{previous_support}")
      return
    else
      Rails.logger.debug("OnLegislatorVoteUpdateScoresJob.update_user_legislator_scores - LegislatorVote.new_support: #{newly_saved_legislator_vote.support}, previous_support: #{previous_support}")
    end
  ...
  def update_legislator_district_score(legislator_vote, users, previous_support)
    ...
    current_count_disagreed = bill_user_votes.count { |buv| LegislatorVote::Support::FOR_AGAINST.include?(previous_support) && buv.support != previous_support }
    ...
    new_count_disagreed = bill_user_votes.count { |buv| LegislatorVote::Support::FOR_AGAINST.include?(legislator_vote.support) && buv.support != legislator_vote.support }
    new_count_legislator_abstained = (legislator_vote.support == LegislatorVote::Support::ABSTAIN) ? bill_user_votes.size : 0
    ...
    diff_count_legislator_abstained = current_count_legislator_abstained + (new_count_legislator_abstained - current_count_legislator_abstained)
    diff_count_no_legislator_vote = current_count_no_legislator_vote + (new_count_no_legislator_vote - current_count_no_legislator_vote)
  ...
  def update_user_legislator_scores(legislator_vote, users, previous_support)
    ...
    user_legislator_scores = T.cast(UserLegislator.where(user: users, legislator: legislator_vote.legislator).map(&:user_legislator_score), T::Array[UserLegislatorScore]).compact
    ...
    user_legislator_scores.each do |uls|
      ...
      if previous_support.blank?
        ...
        if legislator_vote.support == user_vote.support
          new_count_agreed += 1
        elsif LegislatorVote::Support::FOR_AGAINST.include?(legislator_vote.support) && legislator_vote.support != user_vote.support
        ...
        if legislator_vote.support == LegislatorVote::Support::ABSTAIN
          ...
          Rails.logger.warn("OnLegislatorVoteUpdateScoresJob.update_user_legislator_scores - SHOULD NOT BE TRIGGERED. user_vote.support == previous_support")
          new_count_agreed += 1
        elsif legislator_vote.support != user_vote.support
          new_count_disagreed += 1
        end
