...
class OnLegislatorVoteUpdateScoresJob < ApplicationJob
  ...
  def update_legislator_district_score(legislator_vote, users, previous_support)
    ...
    current_count_agreed =
      bill_user_votes.count { |buv| buv.support == previous_support }
    current_count_disagreed =
      bill_user_votes.count do |buv|
        LegislatorVote::Support::FOR_AGAINST.include?(previous_support) &&
          buv.support != previous_support
      end
    current_count_legislator_abstained =
      (
        if previous_support == LegislatorVote::Support::ABSTAIN
          bill_user_votes.size
        else
          0
        end
      )
    current_count_no_legislator_vote =
      previous_support.blank? ? bill_user_votes.size : 0

    new_count_agreed =
      bill_user_votes.count { |buv| buv.support == legislator_vote.support }
    new_count_disagreed =
      bill_user_votes.count do |buv|
        LegislatorVote::Support::FOR_AGAINST.include?(
          legislator_vote.support,
        ) && buv.support != legislator_vote.support
      end
    ...
    new_count_no_legislator_vote =
      legislator_vote.support.blank? ? bill_user_votes.size : 0

    diff_count_agreed =
      current_count_agreed + (new_count_agreed - current_count_agreed)
    diff_count_disagreed =
      current_count_disagreed + (new_count_disagreed - current_count_disagreed)
    diff_count_legislator_abstained =
      current_count_legislator_abstained +
        (new_count_legislator_abstained - current_count_legislator_abstained)
    diff_count_no_legislator_vote =
      current_count_no_legislator_vote +
        (new_count_no_legislator_vote - current_count_no_legislator_vote)

    Rails.logger.debug(
      "LegislatorDistrictScore: #{legislator_district_score.id} BEFORE Update:",
    )
    Rails.logger.debug(
    ...
    legislator_district_score.count_legislator_abstained =
      new_count_legislator_abstained
    legislator_district_score.count_no_legislator_vote =
      new_count_no_legislator_vote

    Rails.logger.debug(
      "LegislatorDistrictScore: #{legislator_district_score.id} AFTER Update:",
    )
    Rails.logger.debug(
      JSON.pretty_generate(
        {
    ...
    user_legislator_scores.each do |uls|
      ...
      if previous_support.blank?
        ...
        if legislator_vote.support == user_vote.support
          new_count_agreed += 1
        elsif LegislatorVote::Support::FOR_AGAINST.include?(
              legislator_vote.support,
