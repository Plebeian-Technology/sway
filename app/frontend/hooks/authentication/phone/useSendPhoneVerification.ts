...
export const useSendPhoneVerification = () => {
    const { post, isLoading } = useAxios_NOT_Authenticated_POST_PUT<sway.IValidationResult>("/phone_verification", { method: "post" })

    const send = useCallback((phone: string) => {
        return post({ phone }).then((result) => result?.success).catch(handleError)
    }, [post])
