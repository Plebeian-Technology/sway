...
import { findLocale, get } from "src/utils";
...
export const onUserBillShareUpdateTotal = functions.firestore
    ...
    .onUpdate(
        async (
            ...
            if (!uid || !localeName) {
                logger.error(
                    "could not get uid or locale name, skipping update",
                );
            ...
            const platformUpdated = Object.keys(after.platforms).find(
                (key: string) => {
                    return (
