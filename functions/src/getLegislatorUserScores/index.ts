...
import { CONGRESS_LOCALE_NAME, Support } from "src/constants";
import SwayFireClient from "src/fire";
import { db, firestore } from "src/firebase";
import { isAtLargeLegislator, isCongressLocale } from "src/utils";
...
export const getLegislatorUserScores = functions.https.onCall(
    async (
        ...
        > => {
            ...
            const result = await fireClient.locales().get(locale);
            if (!result?.userCount) return;

            return {
                countAllUsersInLocale: Number(result.userCount.all || 0),
                countAllUsersInDistrict: _isAtLargeLegislator()
                    ? isCongressLocale(locale)
                        ...
                        : Number(result.userCount.all || 0)
                    : Number(result.userCount[legislator.district] || 0),
            };
        };
