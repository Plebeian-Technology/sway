import { Collections, NOTIFICATION_FREQUENCY, NOTIFICATION_TYPE } from "@sway/constants";
...
export const smsResponse = functions.https.onRequest(async (req: Request, res: Response) => {
    const { From, Body }: { From: string; Body: string } = req.body;
    logger.info("Received twilio message with data -", JSON.stringify(req.body, null, 4));
    ...
    if (Body && STOP_WORDS.includes(Body.toLowerCase())) {
        ...
        const uid: string | void = await db
            ...
            .get()
            .then((snaps) => {
                return (snaps.docs.map((d) => d.data()) as sway.IUser[])[0].uid;
            })
            .catch(logger.error);
        if (!uid) {
            logger.error(
        ...
        const settingsRef = db.collection(Collections.UserSettings).doc(uid);
