    ...
    context "a new MARYLAND Bill is created" do
      it "creates new LegislatorVotes for both the house and senate" do
        ...
        create(
          :vote,
          bill:,
          house_roll_call_vote_number: "1397",
          senate_roll_call_vote_number: "1178",
        )

        expect(LegislatorVote.count).to eql(183)
        expect(
          LegislatorVote.where(legislator: president).first&.support,
        ).to eql(LegislatorVote::Support::FOR)
        expect(LegislatorVote.where(legislator: speaker).first&.support).to eql(
        ...
        expect(
          LegislatorVote.where(legislator: delegate).first&.support,
        ).to eql(LegislatorVote::Support::FOR)

        expect(
          LegislatorVote.where(
            bill:,
        ...
        expect(
          LegislatorVote.where(
            bill:,
            support: LegislatorVote::Support::ABSTAIN,
          ).size,
        ).to eql(2)
      end
    end
