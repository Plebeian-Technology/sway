...
module SeedPreparers
    module Legislators
        class Sway < Base
            ...
            def external_id
                @external_id ||= json.fetch("external_id", json.fetch("externalId", nil))
            end
            ...
            def postal_code
                @postal_code ||=
                    json.fetch(
                        "postal_code",
                        json.fetch("postalCode", json.fetch("zip", json.fetch("zip_code", json.fetch("zipCode", nil)))),
                    )
            end

            def photo_url
                @photo_url ||= json.fetch("photoURL", json.fetch("photo_url", json.fetch("photoUrl", nil)))
            end
