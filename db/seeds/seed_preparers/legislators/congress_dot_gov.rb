    ...
    class CongressDotGov < Base
      ...
      def party
        ...
        par =
          json.fetch(
            "partyName",
            json.dig("partyHistory", 0, "partyAbbreviation"),
          )
        par =
          if json.dig("terms", "item").last&.fetch("chamber", nil) == "Senate"
            "Sen."
      ...
      def first_name
        json.fetch(
          ...
          json.fetch(
            "firstName",
            json.fetch("name", "").split(",").last&.strip,
          ),
        )
      end
      ...
      def last_name
        json.fetch(
          ...
          json.fetch(
            "lastName",
            json.fetch("name", "").split(",").first&.strip,
          ),
        )
      end
      ...
      def link
        ...
        json.fetch("url", json.fetch("officialWebsiteUrl", default_link)) ||
          default_link
      end
          ...
          begin
            ...
            if existing.present?
              ...
              existing.update(
                active: true,
                photo_url:
                  existing.photo_url || result.dig("depiction", "imageUrl"),
                phone:
                  existing.phone ||
                    result.dig("addressInformation", "phoneNumber"),
            ...
            Rails.logger.warn(
              "No connection, skip getting legislator data from congress.gov",
            )
            nil
          end
        end
