...
class SeedBill
    ...
    def self.read_bills(sway_locale)
        T.let(
            JSON.parse(File.read("storage/seeds/data/#{sway_locale.reversed_name.tr("-", "/")}/bills.json")),
    ...
    def seed(json, sway_locale)
        ...
        bill = Bill.find_or_initialize_by(external_id: json.fetch("external_id", nil))
        ...
        if bill.scheduled_release_date_utc.blank? &&
                  Bill.find_by(scheduled_release_date_utc: scheduled_release_date_utc).nil?
