...
class SeedBill
  ...
  def seed(json, sway_locale)
    ...
    bill =
      Bill.find_or_initialize_by(external_id: json.fetch("external_id", nil))
    ...
    bill.senate_vote_date_time_utc =
      json.fetch("senate_vote_date_time_utc", nil)
    ...
    if bill.scheduled_release_date_utc.blank? &&
         Bill.find_by(
           scheduled_release_date_utc: scheduled_release_date_utc,
         ).nil?
